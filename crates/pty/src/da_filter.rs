//! Device-Attributes (DA) query filter for PTY output.
//!
//! Claude Code (and any TUI it runs) can emit DA queries (`CSI c`, `CSI > c`,
//! `CSI = c`) during startup — often BEFORE the frontend xterm has attached, so
//! no terminal is listening to answer them. An unanswered or late-answered DA
//! query desyncs the program and shows up as on-screen artifacts. We intercept
//! the query in the reader thread and reply immediately on the child's behalf,
//! stripping the query from the stream so the frontend xterm never sees (and
//! never double-answers) it. DA *responses* from a nested program pass through
//! untouched, so there's no echo loop.
//!
//! Copied near-verbatim from Terax (github.com/crynta/terax-ai, Apache-2.0).
//! See the repo NOTICE.
